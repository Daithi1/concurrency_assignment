...
void check_result(struct complex ** result, struct complex ** control, int dim1, int dim2)
{
  ...
  for ( i = 0; i < dim1; i++ ) {
    for ( j = 0; j < dim2; j++ ) {
      ...
      // if (diff != 0){
      //   printf("Control: %f\nResult: %f\nDiff: %f\ni: %d\nj: %d\n\n", control[i][j].real, result[i][j].real, diff, i, j);
      // }
      ...
      // if (diff != 0){
      //   printf("Control: %f\nResult: %f\nDiff: %f\ni: %d\nj: %d\n\n", control[i][j].imag, result[i][j].imag, diff, i, j);
      // }
      sum_abs_diff = sum_abs_diff + diff;
    }
  }
...
void team_matmul(struct complex ** A, struct complex ** B, struct complex ** C, int a_rows, int a_cols, int b_cols) {
  ...
  float ** real_a = (float**) malloc(a_rows * sizeof(float*)); //Declaring Cols
  ...
  int i, j;
  for ( i = 0; i < a_rows; i++ ) { //Declaring rows
...
int main(int argc, char ** argv)
{
  ...
  // __m128 a = _mm_set1_ps(5.0);
  // float y = _mm_hadd_ps(a, a);
  // printf("%f  \n", y);
